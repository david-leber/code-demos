...
async fn main() -> Result<()> {
    ...
    if api_key.is_some() {
        tracing::info!("AI assistant initialized with API key");
    } else {
        tracing::info!("AI assistant initialized in simple mode (no API key found)");
    }
    let ai_assistant = AIAssistant::new(api_key);

    // Create app state
    let state = AppState {
        lesson_manager: Arc::new(lesson_manager),
